...
# Compute repository root robustly whether this file lives in scripts/ or scripts/lib/
if [ -z "${ROOT_DIR:-}" ]; then
  if [ -n "${KINDLER_ROOT:-}" ] && [ -d "${KINDLER_ROOT:-}" ]; then
    ROOT_DIR="${KINDLER_ROOT}"
  else
...
provider_for() {
  ...
  # 1. 优先从数据库读取
  if command -v db_get_cluster > /dev/null 2>&1 && db_is_available > /dev/null 2>&1; then
