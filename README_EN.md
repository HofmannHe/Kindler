...
> Official communication is maintained in Chinese. This file is a convenience translation and may lag behind the canonical content described in `openspec/specs/tooling-scripts/spec.md`.

## Language & Communication

- Follow the **Chinese-First Communication** requirement: write official docs, helper output, and commit descriptions in Chinese, keeping command names/identifiers in English.
- When producing English material, include it after the Chinese source and clearly label it as a reference translation only.

## Scripts Overview

- See `scripts/README.md` for categorized entrypoints, libraries, and deprecated wrappers.
- Key commands: `bootstrap.sh`, `cluster.sh` (create/delete/import/status/start/stop/list), `create_env.sh`, `delete_env.sh`, `haproxy_route.sh`, `haproxy_sync.sh`, `reconcile.sh`, `reconcile_loop.sh`, `portainer.sh`, `argocd_register.sh`, `smoke.sh`. Batch tools moved under `tools/maintenance/`.
...
- Best practice for batch creation: run a single reconciliation step after all parallel creates finish:
  ```bash
  ./scripts/reconcile_loop.sh --once   # wraps reconcile.sh --from-db, ApplicationSet sync, HAProxy prune
  ```
...
### Declarative Lifecycle (Clean → Bootstrap → Reconcile → Validate)

The SQLite database is the single source of truth. Every destructive operation **must** be followed by a reconciliation pass so that actual clusters match `clusters` table rows.
...
4. **Validate**
   ...
   - `scripts/db_verify.sh --json-summary` now returns `0` (healthy), `10` (missing clusters), or `11` (state drift) and emits `DB_VERIFY_SUMMARY=…`.
...
```bash
./scripts/test_data_consistency.sh --json-summary
./scripts/db_verify.sh --json-summary        # add --cleanup-missing to prune stale rows
```

Run these after manual tweaks to align SQLite, ApplicationSet, Portainer, and HAProxy before re-running the regression harness.
