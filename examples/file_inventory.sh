...
# Description: Validate that Markdown docs under examples/ are registered in examples/FILE_INVENTORY.md.
# Usage: examples/file_inventory.sh [--check]
# Category: inventory
# Status: stable
# See also: examples/FILE_INVENTORY.md
...
mapfile -t declared_paths < <(
  sed -n '/^## examples\/ 目录文档/I,$p' "$INVENTORY_FILE" \
    | grep '^|' \
    | sed -E 's/^\| *`?([^`]+)`?.*/\1/' \
    | sed 's/^ *//;s/ *$//' \
    | grep -v '^Path' || true
...
# 扫描 examples/ 及其一级子目录下的 Markdown 文件
mapfile -t repo_md < <(
...
die "未登记文档需要被分类为：核心示例文档（补充清单）/ 历史案例（迁移到 docs/history）/ 废弃（删除）"
