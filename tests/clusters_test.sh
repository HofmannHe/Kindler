...
for cluster in $all_clusters; do
  ...
  if [ "$cluster" = "devops" ]; then
    ...
    # 排除: helm-install-* (Jobs), local-path-provisioner, metrics-server, svclb-* (可选组件)
    failed_pods=$(kubectl --context "$ctx" get pods -n kube-system --no-headers 2>/dev/null | grep -v "helm-install-" | grep -v "local-path-provisioner" | grep -v "metrics-server" | grep -v "svclb-" | grep -Ev "Running|Completed" 2>/dev/null | wc -l 2>/dev/null | tr -d ' \n' 2>/dev/null || true)
    # 清理并确保是有效数字
