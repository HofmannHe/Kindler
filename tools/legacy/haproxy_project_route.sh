...
# DEPRECATED: Prefer environment-level routing via scripts/haproxy_route.sh
echo "[DEPRECATED] haproxy_project_route.sh is deprecated. Prefer: scripts/haproxy_route.sh add <env> --node-port <port>" >&2
...
main() {
  local command="$1" project="$2"; shift 2
  ...
  [ -n "$env" ] || { echo "错误: 必须指定 --env 参数" >&2; usage; }
  case "$command" in
    add) add_project_route "$project" "$env" "$node_port" ;;
    remove) remove_project_route "$project" "$env" ;;
    *) echo "未知命令: $command" >&2; usage ;;
  esac
}
