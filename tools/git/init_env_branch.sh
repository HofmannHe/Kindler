...
source "$ROOT_DIR/scripts/lib/lib_config.sh"
...
main() {
	...
	if branch_exists "$env" "$git_repo"; then
		...
		echo "[ERROR] Use 'scripts/lib/lib_git.sh update' to modify existing configuration" >&2
		exit 1
	fi
...
"
	...
	echo "     bash scripts/lib/lib_git.sh read $env $git_repo"
	echo ""
