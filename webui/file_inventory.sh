...
# Description: Validate that Markdown docs under webui/ are registered in webui/FILE_INVENTORY.md.
# Usage: webui/file_inventory.sh [--check]
# Category: inventory
# Status: stable
# See also: webui/FILE_INVENTORY.md
...
mapfile -t declared_paths < <(
  sed -n '/^## webui\/ 目录文档与子目录/I,$p' "$INVENTORY_FILE" \
    | grep '^|' \
    | sed -E 's/^\| *`?([^`]+)`?.*/\1/' \
    | sed 's/^ *//;s/ *$//' \
    | grep -v '^Path' || true
...
# 扫描 webui/ 及其一级子目录下的 Markdown 文件
mapfile -t repo_md < <(
...
die "未登记文档需要被分类为：核心 WebUI 文档（补充清单）/ 历史案例（迁移到 docs/history）/ 废弃（删除）"
